...
function drawChar( args ){
	...
	renderLayer( chars[n], [sz,sz], [x1,y], C );
	renderLayer( chars[n], [sz,sz], [x2,y], C );
}
function buildGrid(){
	var n = cd.nchars;
...
function render(){
	...
	var sz = to1(nums[1])*2;
	for(i=0;i<n;i++){
		canvasAction( drawChar, nums[i]%chars.length, sz, i );
	}
}
function init(){
	nums = getNums();
